...
const ArticlePageSidebar = () => {
  const [sidebar, setSidebar] = useState(false)

  const accordion = () => {
    setSidebar(!sidebar)
  }
  const [isLocalFlyer, setLocalFlyer] = useState(true)

  useEffect(() => {
    if (isBrowser) {
      window.location.pathname.includes('canada-flyer')
        ? setLocalFlyer(false)
        : setLocalFlyer(true)
    }
  }, [setLocalFlyer])
  ...
  return (
    <div className={'side-bar'}>
      <div className={'bread-crumb'}>
        ...
        <Link className={'home'} to={'/mattress-sale-flyers/'}>
          Flyers
        </Link>
        <GSIcon icon={'gs-chevron-right'} />
        <span>{isLocalFlyer ? 'Local Flyer' : 'Canada Flyer'}</span>
      </div>
