...
import getMyVariant from '~/components/functions/get-my-variant'
...
const ProductBox = ({ collectionTitle, product }) => {
  const [toggleLabel, setToggleLabel] = useState('more'),
    ...
    collectionVariant = getMyVariant(product.variants, collectionTitle),
    queenVariant = getQueenVariant(product.variants),
    displayVariant = collectionVariant
      ? collectionVariant.node
      : queenVariant
      ? queenVariant.node
      : initialVariant.node
      ...
      <div className={'box'}>
        ...
        <Link
          ...
          to={`/products/${product.handle}${
            collectionVariant ? `?size=${collectionTitle.toUpperCase()}` : ''
          }`}
        >
            ...
            <div className={'content'}>
              ...
              <div className={'info'}>
                ...
                <div className={'flex-shrink-1 my-auto text-right'}>
                  <h6 className={'price-title'}>OUR PRICE</h6>
                  <h6
                    className={'price-title'}
                  >{`[${displayVariant.selectedOptions[0].value.replace(
                    ' ',
                    ''
                  )}]`}</h6>
                  <h3 className={'price'}>
